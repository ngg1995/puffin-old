...
class Python3Writer(ParseTreeListener):
    ...
    def __init__(self, output,uncerts = {},translations = {},changes={},dependencies = pd.DataFrame()):
        self.output = output
    ...
    def exitArith_expr(self, ctx:Python3Parser.Arith_exprContext):
        ...
        if len(args) != 1:
            ...
            while len(args) != 1:
                ...
                if nargs[0] in self.dependencies.index and nargs[2] in self.dependencies.index:
                    methodtext = ',method = \'%s\'' %self.dependencies.loc[nargs[0],nargs[2]]
                else:
                    methodtext = ''


                if nargs[1] == "+":
                    text = 'puffin.add(%s,%s%s)' %(nargs[0],nargs[2],methodtext)
                elif nargs[1] == "-":
                    text = 'puffin.sub(%s,%s%s)' %(nargs[0],nargs[2],methodtext)
    ...
    def exitTerm(self, ctx:Python3Parser.TermContext):
        ...
        if len(args) != 1:
            ...
            while len(args) != 1:
                ...
                if nargs[1] == "*":
                    text = 'puffin.mul(%s,%s%s)' %(nargs[0],nargs[2],methodtext)
                elif nargs[1] == "/":
                    text = 'puffin.div(%s,%s%s)' %(nargs[0],nargs[2],methodtext)
